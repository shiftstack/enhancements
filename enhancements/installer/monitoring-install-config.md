---
title: monitoring-install-config
authors:
  - "@pierreprinetti"
reviewers:
  - "@mandre for OpenStack-as-infra"
  - "@danielmellado for OpenShift monitoring"
approvers:
  - "@patrickdillon"
...
# Configure monitoring at install-time

## Summary

openshift-monitoring-operator

Give OpenShift administrators the ability to configure some behaviour of 

Add an optional platform-independent section to `install-config.yaml` to
configure the cluster monitoring operator at install-time.
...
All of the following user stories can be addressed today by editing the
`cluster-monitoring-config` configmap in the `openshift-monitoring` namespace
of OpenShift as a day-2 operation. The examples below describe cases where it
would be more ergonomic for customers to be able to set those configuration
options at install-time.

> "As an _administrator of ephemeral OpenShift clusters_, I want to _set up
> Remote Write_ so that I can _visualize metrics and set alerts on an external,
> long-running metrics server_."

> "As an _OpenShift administrator_, I want to _set up Remote Write_ so that I
> can _visualize and set alerts on my OpenShift cluster(s) together with other
> observed systems_."

> "As an _OpenShift administrator_, I want to _set up Remote Write_ so that I
> can _minimize block-storage consumption on my OCP cluster(s) while keeping
> arbitrary long retention on a centralized, external metrics server_."

> "As an _administrator of OpenShift on OpenStack_, I want to _automatically
> have my OpenShift (workload) and OpenStack (infrastructure) metrics collected
> centrally_ so that I can _respond more quickly to incidents by having all the
> data in one place_."

### Goals

Enabling the user to configure some aspects of the cluster-monitoring-operator
behaviour before the cluster is installed.
